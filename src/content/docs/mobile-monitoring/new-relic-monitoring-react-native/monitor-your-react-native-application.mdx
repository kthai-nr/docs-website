...
To set up [custom managed workflow](https://docs.expo.dev/introduction/managed-vs-bare/#bare-workflow) with our agent, run the following command to install our package and add the config plugin to the plugins array of your `app.json` or `app.config.js`.
...
If you are running your app through Expo Go, note that the agent uses native code and will require running your app outside of Expo Go. You can follow the instructions in the [Custom native code in Expo Go](https://docs.expo.dev/bare/using-expo-client/) guide on how to do this.
